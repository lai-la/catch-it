...
Position pos(0,0);
Position pickUp(random(0,8),random(0,8));
...
void setup() {
  ...
  lc.setLed(0,pickUp.getR(),pickUp.getC(),true);
...
void catchIt(){
  int posR = pickUp.getR();
  int posC = pickUp.getC();
  Position caught(posR,posC);

  if(caught.getR()==pos.getR()&&caught.getC()==pos.getC()){
    lc.setLed(0,caught.getR(),caught.getC(),false);
    pickUp.setR(random(0,8));
    pickUp.setC(random(0,8));
    lc.setLed(0,pickUp.getR(),pickUp.getC(),true);
  }
...
void loop() {
  ...
  Serial.print("PickUp: R:");
  Serial.print(pickUp.getR());
  Serial.print(", C:");
  Serial.print(pickUp.getC());
  Serial.print("/n");
